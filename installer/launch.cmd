...
if not exist "%APP_DIR%bin\web_server.exe" (
  set "FAIL_MSG=web_server.exe not found under %APP_DIR%bin. Reinstall the tool."
  goto :fail
)

if not exist "%DATA_ROOT%" mkdir "%DATA_ROOT%" >nul 2>&1
if not exist "%DATA_ROOT%" (
  set "FAIL_MSG=could not create %DATA_ROOT%."
  goto :fail
)

if not exist "%DATA_ROOT%\logs" mkdir "%DATA_ROOT%\logs" >nul 2>&1
if not exist "%DATA_ROOT%\logs" (
  set "FAIL_MSG=could not create %DATA_ROOT%\logs."
  goto :fail
)
...
if %ERRORLEVEL% GEQ 8 (
  set "FAIL_MSG=seeding data into %DATA_ROOT%\data failed, robocopy exit code %ERRORLEVEL%."
  goto :fail
)

if not exist "%DATA_ROOT%\data\working" mkdir "%DATA_ROOT%\data\working" >nul 2>&1
if not exist "%DATA_ROOT%\data\working" (
  set "FAIL_MSG=could not create %DATA_ROOT%\data\working."
  goto :fail
)
...
:fail
rem Quoted so a path containing &, ^, ( or ) in FAIL_MSG prints literally;
rem goto :fail exits the script outright rather than returning to the
rem caller, unlike a bare call, which would need an explicit exit after it.
echo ERROR: "%FAIL_MSG%"
pause
exit /b 1
